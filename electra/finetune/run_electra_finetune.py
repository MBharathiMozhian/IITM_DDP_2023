...
os.environ['HOME'] = "/scratch/scratch6/bharathimozhian/bpe_pubchem112m/electra_mlm/finetune"
...
wandb.init(project='finetune_electra100k')
...
def prun_state_dict(model_dir):
    ...
    state_dict_path = os.path.join(model_dir)
...
def finetune_single_dataset(dataset_name, dataset_type, run_dir, is_molnet):
    ...
    # Using SMILES tokenizer
    tokenizer = SmilesTokenizer(finetune_params.smiles_tokenizer_vocab_path)

    finetune_datasets = get_finetune_datasets(dataset_name, tokenizer, is_molnet)
    
    print('Pretrained model: ', finetune_params.pretrained_model_name_or_path_electra100k)

    if finetune_params.pretrained_model_name_or_path_electra100k:
        # config = ElectraConfig.from_pretrained(
        #     finetune_params.pretrained_model_name_or_path_electra100k)
        # config = ElectraConfig.from_pretrained(f'google/electra-{finetune_params.size}-discriminator')
        config = AutoConfig.from_pretrained(finetune_params.pretrained_model_name_or_path_electra100k)
            # generator_size_divisor = [4, 3, 4][0] # small - 0, base - 1, large - 2
    ...
    if dataset_type == "classification":
        ...
        print('Only classification tasks permitted')

    state_dict = prun_state_dict(finetune_params.pretrained_model_name_or_path_electra100k)
    ...
    training_args = TrainingArguments(
        ...
        save_total_limit=finetune_params.save_total_limit,
...
#model params
pretrained_model_name_or_path_electra100k = finetune_params.pretrained_model_name_or_path_electra100k
...
smiles_tokenizer_vocab_path = finetune_params.smiles_tokenizer_vocab_path
max_tokenizer_len = 128


if pretrained_model_name_or_path_electra100k:
    print("Instantiating pretrained model from: {}".format(pretrained_model_name_or_path_electra100k))
